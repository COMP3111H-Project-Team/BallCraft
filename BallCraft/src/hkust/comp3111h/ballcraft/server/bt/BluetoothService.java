...
public abstract class BluetoothService {
    ...
    /**
     * Call this method to start bluetooth device discovery. Should only be
     * called after bluetooth has been enabled.
     * 
     * @param parent
     *            The activity that is used during discovery
     */
    public void startDiscovery(Activity parent) {
        this.parent = parent;
        parent.registerReceiver(broadcastReceiver, new IntentFilter(
    ...
    /**
     * Should be called before the activity that registered the broadcast
     * receiver is closed.
     */
    void destroy() {
    ...
    /**
     * Asynchronous connection method, connection to a given server using the
    ...
    public BluetoothChannel connectToDevice(BluetoothDevice device)
        ...
        final BluetoothSocket socket = device
                .createRfcommSocketToServiceRecord(getUUID());
    ...
    /**
     * @return The UUID that the server and the client are using. Should be the
     *         same for both the server and the client.
     */
    abstract UUID getUUID();

    /**
     * Asynchronous callback method, called during device discovery when a
     * device that previously is not paired is found.
     * 
     * @param device
     *            The device found
     */
    ...
    /**
     * Asynchronous callback method, called when the device discovery just
     * really started.
     */
    abstract void onDiscoveryStarted();

    /**
     * Asynchronous callback method, called when there is no more device left to
     * be discovered.
     */
    abstract void onDiscoveryFinished();

    /**
     * Asynchronous callback method, called when the server has just started
     * listening.
     */
    ...
    /**
     * @param socket
     *            The client socket to be accepted
