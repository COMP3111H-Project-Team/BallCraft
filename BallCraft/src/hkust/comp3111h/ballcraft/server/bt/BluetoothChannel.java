...
public abstract class BluetoothChannel {
    ...
    private Thread messageReceiver = new Thread() {
        ...
        void receiveOnce() throws IOException {
            ...
            while (numBytesRead < messageLength) {
                numBytesRead += rfile.read(message, numBytesRead, messageLength
                        - numBytesRead);
            }
            onMessage(message);
        }
    ...
    /**
     * Asynchronous method that sends the given message through the socket, to
     * the remote host.
     * 
     * @param message
    ...
    /**
     * Asynchronous callback method, called when the channel is able to send
     * message.
     */
    ...
    /**
     * Asynchronous callback method, called when an error is occurred. Currently
     * it can only occur when the client failed to connect to a server.
     * 
     * @param e
